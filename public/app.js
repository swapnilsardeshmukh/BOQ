...
document.getElementById("analyze_gpt").addEventListener("click", async ()=>{
  ...
  if(!r.ok) return document.getElementById("result").innerText = "Error: "+(r.error||JSON.stringify(r));
  out = calculateTotals(r.result.result || r.result);   // <<< FIX
  renderResultGPT(out);   
});

function updateQty(type, index, value) {
  out[type][index].QTY = Number(value) || 1;   // set qty
  out = calculateTotals(out);                  // recalc totals & amount
  renderResultGPT(out);                        // redraw UI
}
...
function calculateTotals(out) {
  ...
  out.material.forEach(m => {
    m.QTY = Number(m.QTY) || 1;            // default qty
    m.AMOUNT = (Number(m.RATE) || 0) * m.QTY;
  });
  out.totalA = out.material.reduce((sum, m) => sum + m.AMOUNT, 0);

  // LABOUR TOTAL (B)
  out.work.forEach(w => {
    w.QTY = Number(w.QTY) || 1;
    w.AMOUNT = (Number(w.RATE) || 0) * w.QTY;
  });
  out.totalB = out.work.reduce((sum, w) => sum + w.AMOUNT, 0);

  // MISC TOTAL (C)
  out.misc.forEach(m => {
    m.CALCULATED = out.totalA * (Number(m.PERCENTAGE) || 0);
    m.AMOUNT = m.CALCULATED;
  });
  out.totalC = out.misc.reduce((sum, m) => sum + m.AMOUNT, 0);
...
function renderResultGPT(out) {
  ...
  // compute totals before rendering (you already have this function)
...
<table>
<tr>
  <th>Code</th><th>Items</th><th>Class</th><th>UOM</th><th>Rate</th><th>Qty</th><th>Amount</th>
</tr>`;

(out.material || []).forEach((m, i) =>
  html += `<tr>
    <td>${m.CODE}</td>
    <td>${m.ITEMS}</td>
    <td>${m.CLASS}</td>
    <td>${m.UOM}</td>
    <td>${m.RATE}</td>
    <td><input type="number" min="1" value="${m.QTY}" onchange="updateQty('material', ${i}, this.value)" /></td>
    <td>${m.AMOUNT.toFixed(2)}</td>
  </tr>`
);

html += `
<tr><td colspan="6"><b>Total A</b></td><td><b>${out.totalA.toFixed(2)}</b></td></tr>
</table><br>`;
...
<table>
<tr>
  <th>Code</th><th>Description</th><th>%</th><th>Overall</th><th>Calculated (C)</th><th>Amount</th>
</tr>`;
...
html += `
<tr><td colspan="5"><b>Total C</b></td><td><b>${out.totalC.toFixed(2)}</b></td></tr>
</table><br>`;
...
<table>
<tr>
  <th>Code</th><th>Items</th><th>UOM</th><th>Rate</th><th>Qty</th><th>Amount</th>
</tr>`;

(out.work || []).forEach((w, i) =>
  html += `<tr>
    <td>${w.CODE}</td>
    <td>${w.ITEMS}</td>
    <td>${w.UOM}</td>
    <td>${w.RATE}</td>
    <td><input type="number" min="1" value="${w.QTY}" onchange="updateQty('work', ${i}, this.value)" /></td>
    <td>${w.AMOUNT.toFixed(2)}</td>
  </tr>`
);

html += `
<tr><td colspan="5"><b>Total B</b></td><td><b>${out.totalB.toFixed(2)}</b></td></tr>
</table><br>`;



  // ---------------------------------------------------------------------------------------
  // FINAL OVERALL TOTAL (CURRENTLY FROM BACKEND)
  // ---------------------------------------------------------------------------------------
  html += `
<h3>Summary</h3>
<table>
<tr><td>Total A (Materials)</td><td><b>${out.totalA.toFixed(2)}</b></td></tr>
<tr><td>Total B (Labour)</td><td><b>${out.totalB.toFixed(2)}</b></td></tr>
<tr><td>Total C (Misc)</td><td><b>${out.totalC.toFixed(2)}</b></td></tr>
<tr><td><h2>OVERALL (A + B + C)</h2></td><td><h2><b>${out.totalOverall.toFixed(2)}</b></h2></td></tr>
</table>
`;
